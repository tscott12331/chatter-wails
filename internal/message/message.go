...
func SendMessage(userId string, access_token string, brdId string, message string, replyId *string) (*api.ApiPostMessagesData, error) {
	body := api.ApiPostMessagesBody{
		Sender_id:      userId,
	...
	res, err := api.ApiPostMessages(access_token, body, map[string][]string{})
	if err != nil {
	...
	if res.Status != 200 {
		...
		return nil, &api.StatusError[api.ApiPostMessagesRes]{ Res: res }
	}
